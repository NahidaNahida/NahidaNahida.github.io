...
The site also supports Google Analytics, following the common AcadHomepage setup. Set `analytics.googleAnalyticsId` in `site-config.js` to your GA measurement id. For a visible visitor map below `Page visits` on the home page, set `visits.clustrmapsScriptUrl` to the map or globe script URL generated by ClustrMaps, for example `//clustrmaps.com/globe.js?d=YOUR_CLUSTRMAPS_ID`.
