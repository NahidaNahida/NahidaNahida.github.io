...
The site also supports Google Analytics, following the common AcadHomepage setup. Set `analytics.googleAnalyticsId` in `site-config.js` to your GA measurement id. For a visible visitor map below `Page visits` on the home page, set `visits.clustrmapsScriptUrl` to the map or globe script URL generated by ClustrMaps. The default uses the 2D map widget, for example `//cdn.clustrmaps.com/map_v2.js?cl=ffffff&w=240&t=tt&d=YOUR_CLUSTRMAPS_ID`; the globe widget also works with `//clustrmaps.com/globe.js?d=YOUR_CLUSTRMAPS_ID`.
